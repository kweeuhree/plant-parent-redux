...
const PlantForm = ({ formMode }: Props) => {
  ...
  useEffect(() => {
    ...
    formMode === "ADD" && hasPlants && dispatch(unselectPlant());
  }, [formMode, hasPlants, dispatch]);
  ...
  const initialState =
    formMode === "ADD"
      ? { name: "", image: "" }
      : { name: plant?.name, image: plant?.image.imageUrl };
  ...
  const updatePlantWrapper = async (newImage: Image, newName: string) => {
    console.log(
      `Updating plant with ${newImage.imageId} and ${newImage.imageUrl} `,
    );
    ...
    try {
      const plantIsUpdated = await updatePlantReq(
        newImage,
        plant.plantId,
        newName,
        plant.dateCreated,
